...
import {Borrower, CreditLine, ERC20, TranchedPool} from "../types"
...
describe("Borrower", async () => {
  ...
  describe("drawdown", async () => {
    ...
    it("should let you drawdown the amount", async () => {
      await expectAction(() =>
        borrowerContract.drawdown(tranchedPool.address, bnToHex(amount), borrower, {from: borrower})
      ).toChange([[async () => await getBalance(borrower, usdc), {by: amount}]])
    })

    it("should not let anyone except the borrower drawdown", async () => {
      return expect(
        borrowerContract.drawdown(tranchedPool.address, bnToHex(amount), person3, {from: person3})
      ).to.be.rejectedWith(/Must have admin role/)
    })

    it("should not let anyone except the borrower drawdown via oneInch", async () => {
      return expect(
        borrowerContract.drawdownWithSwapOnOneInch(tranchedPool.address, bnToHex(amount), person3, usdc.address, bnToHex(amount), [], {
          from: person3,
        })
      ).to.be.rejectedWith(/Must have admin role/)
    })
    ...
    describe("address forwarding", async () => {
      it("should support forwarding the money to another address", async () => {
        await expectAction(() =>
          borrowerContract.drawdown(tranchedPool.address, bnToHex(amount), person3, {from: borrower})
        ).toChange([
      ...
      context("addressToSendTo is the zero address", async () => {
        it("should default to msg.sender", async () => {
          await expectAction(() =>
            borrowerContract.drawdown(tranchedPool.address, bnToHex(amount), ZERO_ADDRESS, {from: borrower})
          ).toChange([
      ...
      context("addressToSendTo is the contract address", async () => {
        it("should default to msg.sender", async () => {
          await expectAction(() =>
            borrowerContract.drawdown(tranchedPool.address, bnToHex(amount), borrowerContract.address, {from: borrower})
          ).toChange([
            [async () => await getBalance(borrower, usdc), {by: amount}],
            [async () => await getBalance(borrowerContract.address, usdc), {by: new BN(0)}],
          ])
    ...
    describe("transfering ERC20", async () => {
      ...
      it("should even allow transfers not to the borrower themselves", async () => {
        ...
        // Send that money to the borrower!
        await expectAction(() =>
          borrowerContract.transferERC20(usdc.address, person3, bnToHex(amount), {from: borrower})
      ...
      it("should only allow admins to transfer the money", async () => {
        ...
        // Send that money to the borrower!
        return expect(
          borrowerContract.transferERC20(usdc.address, person3, bnToHex(amount), {from: person3})
        ).to.be.rejectedWith(/Must have admin role/)
      })
    })
  })
