...
describe("Index Staking Pool", () => {
  const setUp = deployments.createFixture(
    async () => {
      ...
      let NAOSConfig = await ethers.getContractFactory("NAOSConfig");
      ...
      await nBUSDNAOSConfig.setAddress(2, nBUSD.address);
      ...
      await stakingPool.connect(governance).createPool(nUSDCNAOSConfig.address);

      return {
        nUSDC, nBUSD, NAOS, nUSDCNAOSConfig, nBUSDNAOSConfig, boostPool, stakingPool, deployer, governance, user1, user2
      };
    })
  ...
  let calcUserWeight = (
    ...
    if (boostPoolDepositedWeight > 0) {
      ...
      if (weighted.sub(userDeposited) > 0) {
  ...
  describe("set parameters", () => {
    context("set pools", () => {
      ...
      it("it successfully creates the second pools", async () => {
        await deployment.stakingPool.connect(deployment.governance).createPool(deployment.nBUSDNAOSConfig.address);
  ...
  describe("deposit status", () => {
    ...
    it('it should be rejected if there is no enough tokens', async () => {
      await expect(
        deployment.stakingPool.connect(deployment.user1).deposit(0, user1NUSDCDepositAmount.add(1))
      ).to.be.revertedWith("ERC20: transfer amount exceeds balance");
    })

    it("it successfully deposits", async () => {
      await deployment.stakingPool.connect(deployment.user1).deposit(0, user1NUSDCDepositAmount);
      await deployment.stakingPool.connect(deployment.user1).deposit(1, user1NBUSDDepositAmount);
      await deployment.stakingPool.connect(deployment.user2).deposit(0, user2NUSDCDepositAmount);
      await deployment.stakingPool.connect(deployment.user2).deposit(1, user2NBUSDDepositAmount);
    ...
    describe("withdraw status", () => {
      before(async () => {
        ...
        await deployment.stakingPool.connect(deployment.user1).deposit(1, user1NBUSDDepositAmount);
        await deployment.stakingPool.connect(deployment.user2).deposit(0, user2NUSDCDepositAmount);
        await deployment.stakingPool.connect(deployment.user2).deposit(1, user2NUSDCDepositAmount);
      });

      it("it should be rejected if the input index is out of range", async () => {
        await expect(
          deployment.stakingPool.connect(deployment.user1).withdraw(0, [1, 3], [1, 2])
      ...
      it("it should be rejected if the input is invalid", async () => {
        await expect(
          deployment.stakingPool.connect(deployment.user1).withdraw(0, [0], [1, 2])
      ...
      it("it withdraw successfully", async () => {
        await deployment.stakingPool.connect(deployment.user1).withdraw(1, [0], [user1NBUSDDepositAmount]);
        await deployment.stakingPool.connect(deployment.user2).withdraw(0, [0], [user1NUSDCDepositAmount]);

        let pool0Deposited = user1NUSDCDepositAmount.add(user2NUSDCDepositAmount).sub(user1NUSDCDepositAmount);
        let pool1Deposited = user1NBUSDDepositAmount.add(user2NUSDCDepositAmount).sub(user1NBUSDDepositAmount);
        let user1pool0Deposited = user1NUSDCDepositAmount;
        let user1pool1Deposited = user1NBUSDDepositAmount.sub(user1NBUSDDepositAmount);
        let user2pool0Deposited = user2NUSDCDepositAmount.sub(user1NUSDCDepositAmount);
        let user2pool1Deposited = user2NUSDCDepositAmount;
        expect((await deployment.stakingPool.getPoolTotalDeposited(0)).toString()).equal(pool0Deposited);
        expect((await deployment.stakingPool.getPoolTotalDeposited(1)).toString()).equal(pool1Deposited);
        expect((await deployment.stakingPool.getStakeTotalDeposited(0, deployment.user1.address, 0)).toString()).equal(user1pool0Deposited);
        expect((await deployment.stakingPool.getStakeTotalDeposited(0, deployment.user2.address, 0)).toString()).equal(user2pool0Deposited);
        expect((await deployment.stakingPool.getStakeTotalDeposited(1, deployment.user1.address, 0)).toString()).equal(user1pool1Deposited);
        expect((await deployment.stakingPool.getStakeTotalDeposited(1, deployment.user2.address, 0)).toString()).equal(user2pool1Deposited);
        ...
        let order4 = await deployment.stakingPool.getUserStakeOrderByIndex(1, deployment.user2.address, 0);
        expect(order1.totalDeposited).equal(user1pool0Deposited);
        expect(order2.totalDeposited).equal(user2pool0Deposited);
        expect(order3.totalDeposited).equal(user1pool1Deposited);
        expect(order4.totalDeposited).equal(user2pool1Deposited);
        expect(order1.totalDepositedWeight)
          .equal(calcUserWeight(user1NUSDCDepositAmount, user1NUSDCDepositAmount, totalBoostPoolDeposit, user1BoostPoolDeposit));
        expect(order2.totalDepositedWeight)
        ...
        calcUserWeight(user1NBUSDDepositAmount.add(user2NUSDCDepositAmount), user2NUSDCDepositAmount, totalBoostPoolDeposit, user2BoostPoolDeposit)
        expect(order4.totalDepositedWeight)
          .equal(calcUserWeight(user1NBUSDDepositAmount.add(user2NUSDCDepositAmount), user2NUSDCDepositAmount, totalBoostPoolDeposit, user2BoostPoolDeposit));
      })
    })
