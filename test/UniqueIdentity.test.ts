...
const setupTest = deployments.createFixture(async ({deployments}) => {
  ...
  const uninitializedUniqueIdentity = await getContract<TestUniqueIdentity, any>(
    "TestUniqueIdentity",
    ETHERS_CONTRACT_PROVIDER,
    {
...
describe("UniqueIdentity", () => {
  ...
  describe("initialize", () => {
    ...
    //   await uninitializedUniqueIdentity.connect(signer).initialize(owner, UNIQUE_IDENTITY_METADATA_URI)
    ...
    it("cannot be called twice", async () => {
      ...
      // await uninitializedUniqueIdentity.connect(signer).initialize(owner, UNIQUE_IDENTITY_METADATA_URI)
      await expect(
        uninitializedUniqueIdentity.connect(signer).initialize(anotherUser2, UNIQUE_IDENTITY_METADATA_URI)
  ...
  describe("balanceOf", () => {
    it("returns 0 for a non-minted token", async () => {
      ...
      expect(bnToBnjs(await uniqueIdentity.balanceOf(recipient, bnToHex(new BN(0))))).to.bignumber.equal(new BN(0))
    })
  ...
  describe("setURI", () => {
    ...
    it("allows sender who has owner role", async () => {
      ...
      await expect(uniqueIdentity.connect(signer).setURI(NEW_URI)).to.be.fulfilled
    })

    it("rejects sender who lacks owner role", async () => {
      expect(await uniqueIdentity.hasRole(OWNER_ROLE, anotherUser)).to.equal(false)
      const signer = await ethers.getSigner(anotherUser)
      await expect(uniqueIdentity.connect(signer).setURI(NEW_URI)).to.be.rejectedWith(
        /Must have admin role to perform this action/
      )
    })
    ...
    it("updates state but does not emit an event", async () => {
      const tokenId = new BN(0)
      const uriBefore = await uniqueIdentity.uri(bnToHex(tokenId))
      expect(uriBefore).to.equal(UNIQUE_IDENTITY_METADATA_URI)
      const signer = await ethers.getSigner(owner)
      const receipt = await uniqueIdentity.connect(signer).setURI(NEW_URI)
      const uriAfter = await uniqueIdentity.uri(bnToHex(tokenId))
  ...
  describe("mint", () => {
    ...
    describe("requires payment", () => {
      it("rejects insufficient payment", async () => {
        ...
        await expect(
          uniqueIdentity.connect(signer).mint(...mintParams, signature, {
            value: bnToHex(MINT_PAYMENT.sub(new BN(1))),
          })
      ...
      it("accepts overpayment", async () => {
        ...
        await expect(
          uniqueIdentity.connect(signer).mint(...mintParams, signature, {
            value: bnToHex(MINT_PAYMENT.add(new BN(1))),
          })
        ).to.be.fulfilled
      })
    })
