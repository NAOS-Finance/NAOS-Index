  ...
  describe("mint", async () => {
    ...
    context("with real pool validation turned on", async () => {
      ...
      it("should disallow invalidly created pools", async () => {
        ...
        return expect(fakePool.connect(signer).deposit(bnToHex(new BN(1)), bnToHex(usdcVal(5)))).to.be.rejectedWith(/Invalid pool/)
      })
    })
    ...
    it("should use the current rewardsPerPrincipalShare when it's a second drawdown", async () => {
      ...
      await pool.deposit(bnToHex(new BN(2)), bnToHex(amount), { from: person2 })
      await pool.lockJuniorCapital({from: person2})
      await pool.lockPool({from: person2})
      await pool.drawdown(bnToHex(usdcVal(10)), {from: person2})
      ...
      await pool.pay(bnToHex(usdcVal(5)), {from: person2})
    ...
    it("allows minting even after the limit on the PoolInfo", async () => {
      ...
      await expect(pool.deposit(bnToHex(new BN(1)), bnToHex(amountThatPutsUsOver), {from: person2})).to.be.fulfilled
    })
