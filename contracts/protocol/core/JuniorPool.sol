...
contract JuniorPool is BaseUpgradeablePausable, IJuniorPool {
  ...
  function _initializeNextSlice(uint256 newFundableAt) internal {
    uint256 poolSlicesNum = poolSlices.length;
    require(poolSlicesNum < 5, "Cannot exceed 5 slices");
    poolSlices.push(
      PoolSlice({
        seniorTranche: TrancheInfo({
          id: poolSlicesNum.mul(NUM_TRANCHES_PER_SLICE).add(1),
        ...
        juniorTranche: TrancheInfo({
          id: poolSlicesNum.mul(NUM_TRANCHES_PER_SLICE).add(2),
          principalSharePrice: usdcToSharePrice(1, 1),
          interestSharePrice: 0,
          principalDeposited: 0,
          lockedUntil: 0
